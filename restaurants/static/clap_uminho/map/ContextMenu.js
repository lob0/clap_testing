function ContextMenu(map, options){
	...
	this.pixelOffset=options.pixelOffset || new google.maps.Point(5,-5);
}
...
ContextMenu.prototype.draw=function(){
	if(this.isVisible_){
		...
		//alert(left+"-"+top + "\n "+ mapSize.width+" "+ menuSize.width+"\n"+ mapSize.height+" "+menuSize.height+"\n\n"+((mapSize.width/2)-menuSize.width-this.pixelOffset.x));
		if(mousePosition.x>(mapSize.width/2)-menuSize.width-this.pixelOffset.x){
			left=left-(menuSize.width)-this.pixelOffset.x;
		} else {
			left+=this.pixelOffset.x;
		}
		
		if(mousePosition.y>(mapSize.height/2)-menuSize.height-this.pixelOffset.y){
			top= top -(mousePosition.y+menuSize.height-this.pixelOffset.y   -(mapSize.height/2)); //top-menuSize.height-this.pixelOffset.y;
		} else {
